...
DictionaryLoader.prototype.load = function (load_callback) {
    ...
    async.parallel([
        // Trie
        function (callback) {
            async.map([ "base.gz", "check.gz" ], function (filename, _callback) {
        ...
        function (callback) {
            async.map([ "tid.gz", "tid_pos.gz", "tid_map.gz" ], function (filename, _callback) {
                loadArrayBuffer(path.join(dic_path, filename), function (err, buffer) {
                    if(err) {
                        return _callback(err);
                    }
        ...
        function (callback) {
            loadArrayBuffer(path.join(dic_path, "cc.gz"), function (err, buffer) {
                if(err) {
                    return callback(err);
                }
        ...
        function (callback) {
            async.map([ "unk.gz", "unk_pos.gz", "unk_map.gz", "unk_char.gz", "unk_compat.gz", "unk_invoke.gz" ], function (filename, _callback) {
                loadArrayBuffer(path.join(dic_path, filename), function (err, buffer) {
                    if(err) {
                        return _callback(err);
                    }
